...
export function logEvent(eventName_1) {
    return __awaiter(this, arguments, void 0, function* (eventName, eventDetails = {}) {
        if (process.env.FUSIONAUTH_TELEMETRY === "false") {
            return false;
        }
        const config = loadConfig();
        if (config.globalConfig.telemetry) {
            const capture = yield posthogClient.capture({
                distinctId: config.globalConfig.id,
                event: eventName,
                properties: eventDetails
            });
            yield posthogClient.shutdown();
            return true;
        }
        else {
            return false;
